...
func main() {
	var (
		...
		flagSrc     = flag.String("src", "", "kernel source path root")
	)
	arches := make(map[string]*string)
	for arch := range targets.List[targets.CheriBSD] {
		arches[arch] = flag.String("obj-"+arch, "", arch+" kernel object file")
	}
	defer tool.Init()()
	if *flagSrc == "" {
		fmt.Fprintf(os.Stderr, "no kernel source path given\n")
		flag.PrintDefaults()
		os.Exit(1)
	}
	var warnings []Warn
	for arch, obj := range arches {
		...
		warnings1, err := check(*flagOS, arch, *obj, *flagSrc)
		if err != nil {
			tool.Fail(err)
		}
...
func check(OS, arch, obj, src string) ([]Warn, error) {
	var warnings []Warn
	if obj == "" {
		return nil, fmt.Errorf("no object file in -obj-%v flag", arch)
	}
	syscalls, includes, err := parseDescriptions(OS, arch)
	if err != nil {
		return nil, err
	}
	argSizes, _, err := getSyscallArgSizes(syscalls)
	// _, _, err = getSyscallArgSizes(syscalls)
	if err != nil {
		return nil, err
	}
	// for syscallName, status := range inconsistent {
	// 	if status {
	// 		fmt.Printf("syscall %v has inconsistent arg sizes\n", syscallName)
	// 	}
	// }
	var goodSyscalls []string
	for syscallName, _ := range argSizes {
		// fmt.Printf("syscall %v arg sizes %v\n", syscallName, argSize)
		goodSyscalls = append(goodSyscalls, syscallName)
	}

	bin, out, err := compile(OS, arch, src, goodSyscalls, includes)
	if err != nil {
		if out == nil {
			return nil, err
		} else {
	...
	// warnings = append(warnings, warnings1...)
	// if dwarf {
	structs, err := parseKernelObject(bin)
	if err != nil {
		return nil, err
	}
	...
	// 	fmt.Printf("%v\n", v)
	// }
	big_struct := structs["big_syscall_struct"]
	for i, syscall := range goodSyscalls {
		var argsTypes []dwarf.Type
		var dwarfArgsSize []Arg
		args1 := (*big_struct).Field[i].Type
		switch args := args1.(type) {
			case *dwarf.StructType:
				for _, arg := range args.Field {
					if !strings.HasSuffix(arg.Name, "_l_") &&
					   !strings.HasSuffix(arg.Name, "_r_") &&
					   arg.Name != "dummy" {
					   	argsTypes = append(argsTypes, arg.Type)
					}
				}
			default:
				return warnings, fmt.Errorf("argument struct should be struct")
		}
		for _, argType1 := range argsTypes {
			switch argType := argType1.(type) {
				case *dwarf.PtrType:
					// fmt.Printf("points to: %v\n", argType.Type)
					sz := argType.Type.Size()
					// A char * is most likely a string, not a pointer
					// to a singular character
					if sz == 1 {
						sz = 0
					}
					dwarfArgsSize = append(dwarfArgsSize, Arg{
						Size: uint64(sz),
						IsPtr: true,
					})
				default:
					dwarfArgsSize = append(dwarfArgsSize, Arg{
						Size: uint64(argType.Size()),
						IsPtr: false,
					})
			}
		}
		hasStruct := false
		for _, arg := range argSizes[syscall] {
			if arg.Size > 16 {
				hasStruct = true
				break
			}
		...
		if hasStruct {
			if !reflect.DeepEqual(argSizes[syscall], dwarfArgsSize) {
				fmt.Printf("syscall %v has desc: %v, defined: %v\n", syscall, argSizes[syscall], dwarfArgsSize)
			}
		}
	}
...
func parseDescriptions(OS, arch string) ([]prog.Syscall, []string, error) {
	...
	for _, sysc := range prg.Syscalls {
		syscalls = append(syscalls, *sysc)
	}
	return syscalls, includes, nil
}

func getSyscallArgSizes(syscalls []prog.Syscall) (map[string][]Arg, map[string]bool, error) {
	result := make(map[string][]Arg)
	inconsistent := make(map[string]bool)
	for _, syscall := range syscalls {
		if syscall.NR == 0 || strings.Contains(syscall.Name, "freebsd1") {
			// Skip pseudo-syscalls and freebsd1[1,2] compat syscalls
			continue
		}
		realName := strings.Split(syscall.Name, "$")[0]
		bad, ok := inconsistent[realName]
		if !ok || !bad {
			inconsistent[realName] = false
			var currArgsSizes []Arg
				...
				switch a := (arg).Type.(type) {
					case *prog.PtrType:
						if a.Elem.Varlen() {
							...
							argSize.Size = a.Elem.Size()
						}
						argSize.IsPtr = true
				}
				currArgsSizes = append(currArgsSizes, argSize)
			}
			if !inconsistent[realName] {
				argsSizes, ok1 := result[realName]
				if ok1 {
					if !reflect.DeepEqual(argsSizes, currArgsSizes) {
						inconsistent[realName] = true
						delete(result, realName)
					}
				} else {
					result[realName] = currArgsSizes
				}
			}
		}
	}
	return result, inconsistent, nil
}

func prepareArch(srcDir, buildDir string) error {
	// Always arm64 regardless of hybrid or purecap
	archName := "arm64"
	includeDir := filepath.Join(srcDir, "sys", archName, "include")
	...
	if err := os.Symlink(absIncludeDir,
		filepath.Join(buildDir, "machine")); err != nil {
...
func compile(OS, arch, srcDir string, goodSyscalls, includes[]string) (string, []byte, error) {
	src := new(bytes.Buffer)
	tmplArgs := TemplateArgs{
		Syscalls: goodSyscalls,
		Includes: includes,
	}
	if err := syscallsStruct.Execute(src, tmplArgs); err != nil {
		return "", nil, err
	}
	...
	buildDir, err := os.MkdirTemp("", "syzkaller-check-build")
	if err != nil {
		return "", nil, fmt.Errorf("failed to create temp dir: %w", err)
	}
	err = prepareArch(srcDir, buildDir)
	if err != nil {
		return "", nil, err
	}
	target := targets.Get(OS, arch)
	args := target.CFlags
	args = append(args, []string{
		"-I", filepath.Join(srcDir, "sys"),
		"-I", filepath.Join(srcDir, "sys"),
		"-I", filepath.Join(srcDir, "sys", "sys"),
		"-I", filepath.Join(srcDir, "sys", "contrib", "ck", "include"),
		"-I", filepath.Join(srcDir, "include"),
		"-I", filepath.Join(buildDir),
		"-D_WANT_FILE",
		"-x", "c", "-",
		"-o", binFile,
		"-w", "-g",
	}...)
	// fmt.Printf("compiler args: %v\n", args)
	cmd := osutil.Command(target.CCompiler, args...)
	cmd.Stdin = src
...
var syscallsStruct = template.Must(template.New("").Parse(`
#include <sys/sysproto.h>
{{range $include := $.Includes}}#include <{{$include}}>
{{end}}

struct big_syscall_struct {
{{range $i, $sysc := $.Syscalls}}	struct {{$sysc}}_args field{{$i}};
{{end}}};

int main() {
	struct big_syscall_struct beeg;
}
`))