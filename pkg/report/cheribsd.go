...
func ctorCheribsd(cfg *config) (reporterImpl, []string, error) {
	ctx := &cheribsd{
		config: cfg,
	}
	return ctx, nil, nil
...
func (ctx *cheribsd) ContainsCrash(output []byte) bool {
	return containsCrash(output, cheribsdOopses, ctx.ignores)
}

func (ctx *cheribsd) Parse(output []byte) *Report {
	return simpleLineParser(output, cheribsdOopses, cheribsdStackParams, ctx.ignores)
}

func (ctx *cheribsd) Symbolize(rep *Report) error {
	return nil
}

var cheribsdStackParams = &stackParams{}

// nolint: goconst
var cheribsdOopses = append([]*oops{
	{
		[]byte("Fatal trap"),
		[]oopsFormat{
			{
